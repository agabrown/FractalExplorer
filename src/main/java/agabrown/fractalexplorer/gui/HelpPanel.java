...
public final class HelpPanel extends JPanel {
    ...
    private static final String[][] HELP_DATA = {{"h", "Toggle help screen visible"},
            ...
            {"c", "Toggle between colour scale and black and white"},
     ...
     * positioning.
     *
     * @param g2d Graphics context used for drawing.
     * @param frc Font render context for the text.
     */
    private void drawHelpTable(final Graphics2D g2d, final FontRenderContext frc) {
        ...
        textBox.draw(g2d, (float) this.getWidth() / 6, 3 * lineHeight);
        textBox = new TextLayout(COLUMN_NAMES[1], textFont, frc);
        textBox.draw(g2d, (float) this.getWidth() / 9 * 4, 3 * lineHeight);
        ...
        for (int i = 0; i < HELP_DATA.length; i++) {
            ...
            textBox.draw(g2d, (float) this.getWidth() / 6, (currentLine + i) * lineHeight);
            textBox = new TextLayout(HELP_DATA[i][1], textFont, frc);
            textBox.draw(g2d, (float) this.getWidth() / 9 * 4, (currentLine + i) * lineHeight);
        }
    }
